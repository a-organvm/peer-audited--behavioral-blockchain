...
| Unified beta readiness contract + artifact (`beta-readiness-summary.json`) | Phase 1 release operations | Implemented | `/scripts/smoke/beta-readiness.sh`, `/docs/planning/beta-readiness-contract.md`, `/.github/workflows/ci.yml` | `npm run beta:readiness` emits `artifacts/beta-readiness-summary.json`; CI uploads artifact in `beta_readiness` job | Platform |
